...
export default function PeliculasTab() {
  ...
  const movies = useMemo(
    () => channels.filter((ch) => getChannelType(ch.group_name) === "movies"),
    [channels]
  );
      ...
      {movies.length === 0 ? (
        ...
        >
          <View style={styles.empty}>
            <Text style={styles.emptyText}>No hay películas aún.</Text>
            <Text style={styles.emptyHint}>Añade listas M3U con películas en Configuración.</Text>
          </View>
        </ScrollView>
      ) : (
        <ScrollView
        ...
        >
          <SectionRow title="Películas" items={movies} />
